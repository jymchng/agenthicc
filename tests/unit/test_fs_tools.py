...
import pytest
from agenthicc.tools.fs import (FsToolKit, ReadFileTool, WriteFileTool, AppendFileTool,
    DeleteFileTool, MoveFileTool, CopyFileTool, ListDirectoryTool, MakeDirectoryTool,
    FileExistsTool, SearchFilesTool, GrepFilesTool, GetFileInfoTool, ReadLinesTool, PatchFileTool)
...
class TestWriteFileTool:
    ...
    async def test_traversal_denied(self, tmp_path):
        r = await WriteFileTool().execute({"path": "../../tmp/evil.txt", "content": "x"}, ctx(tmp_path))
        assert r["ok"] is False
...
class TestMoveFileTool:
    async def test_moves(self, tmp_path):
        ...
        r = await MoveFileTool().execute({"source": "src.txt", "destination": "dst.txt"}, ctx(tmp_path))
        assert r["ok"] is True
...
class TestCopyFileTool:
    async def test_copies(self, tmp_path):
        ...
        r = await CopyFileTool().execute({"source": "orig.txt", "destination": "copy.txt"}, ctx(tmp_path))
        assert r["ok"] is True
        assert (tmp_path / "copy.txt").read_text() == "copy me"

class TestListDirectoryTool:
    async def test_lists_files(self, tmp_path):
        (tmp_path / "a.py").write_text(""); (tmp_path / "b.py").write_text("")
    ...
    async def test_pattern_filter(self, tmp_path):
        (tmp_path / "a.py").write_text(""); (tmp_path / "b.txt").write_text("")
...
class TestSearchFilesTool:
    async def test_finds_matching(self, tmp_path):
        (tmp_path / "main.py").write_text(""); (tmp_path / "test.js").write_text("")
...
class TestPatchFileTool:
    async def test_replaces_content(self, tmp_path):
        ...
        r = await PatchFileTool().execute({"path": "p.py", "old_content": "bcrypt", "new_content": "argon2"}, ctx(tmp_path))
    ...
    async def test_old_not_found(self, tmp_path):
        ...
        r = await PatchFileTool().execute({"path": "p.py", "old_content": "NOTHERE", "new_content": "x"}, ctx(tmp_path))
        assert r["ok"] is False
