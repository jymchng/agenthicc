"""WorkflowRunner — executes graph-based workflows (PRD-101)."""
...
class WorkflowRunner:
    """Executes a WorkflowGraph by walking edges until a terminal node or error.

    Parameters
    ----------
    definition:   WorkflowGraph to execute.
    config:       WorkflowConfig holding all session-scoped singletons.
    mode_manager: Optional ModeManager for per-node mode overrides.
    """
    ...
    def __init__(
        ...
        # Gap 1 (lauren-ai): no public model_id on AgentRunnerBase.
    ...
    async def run(self, intent: str) -> None:
        ...
        wf_run   = WorkflowRun(
            ...
            intent=intent,
            current_phase=self._def.entry,
            total_phases=len(self._def.nodes),
        )
        ...
        await self._cfg.processor.emit(Event.create("WorkflowRunStarted", {
            ...
            "workflow_name": self._def.name,
            "intent":        intent,
            "phase_names":   self._def.node_names(),
        }))

        await self._run_graph(intent, data_bus, wf_run, run_id, self._def.entry)

    async def resume(self, context: DataBus) -> None:
        """Resume from a pre-populated DataBus (e.g. restored from kernel log)."""
        from lauren_ai._memory import ShortTermMemory  # noqa: PLC0415
        from agenthicc.workflow.plugin import WorkflowRun  # noqa: PLC0415
        ...
        start_node = self._find_resume_node(context)
        if start_node is None:
            wf_run = dataclasses.replace(wf_run, status="complete")
            self._cfg.app_state.workflow_run.set(wf_run)
            return

        await self._run_graph(context.intent, context, wf_run, run_id, start_node)
    ...
    async def _run_graph(
        ...
        try:
            while node_name is not None:
                node = self._def.get_node(node_name)
                if node is None:
                ...
                total_runs += 1
                _max = self._def.max_total_phase_runs
                if _max > 0 and total_runs > _max:
                    log.warning(
                        "Workflow %s: global cap reached (%d/%d runs).",
                        self._def.name, total_runs, _max,
                    )
                    wf_run = dataclasses.replace(wf_run, status="failed", current_phase=None)
                    self._cfg.app_state.workflow_run.set(wf_run)
                    self._cfg.conv_store.append_event("error", {
                        "message": (
                            f"Workflow '{self._def.name}' stopped after {total_runs} "
                            f"node runs (limit: {_max}). Use /auto to continue manually."
                        )
                    })
                    return
                ...
                if node.parallel_with:
                    siblings = [
                        self._def.get_node(n)
                        for n in ([node_name] + list(node.parallel_with))
                        if self._def.get_node(n) is not None
                    ]
                    ...
                    for sib, result in zip(siblings, outputs):
                        ...
                        wf_run = dataclasses.replace(
                            wf_run,
                            phase_history=wf_run.phase_history + [PhaseRunRecord(
                ...
                node_idx = self._def.node_index(node_name)
                wf_run = dataclasses.replace(
                    wf_run,
    ...
    async def _run_node(self, node: PhaseNode, intent: str, data_bus: DataBus) -> NodeResult:
        ...
        from agenthicc.workflow.phase_tools import make_transition_tools  # noqa: PLC0415
        ...
        # One @tool() per edge, each named after the edge label.
        # __name__/__doc__ set before @_tool() so the LLM schema is correct.
        transition_tools = make_transition_tools(
            node, data_bus, transition_event, transition_data, self._cfg.approval_svc,
        )
        filtered = filtered + transition_tools
    ...
    def _build_node_prompt(self, node: PhaseNode, intent: str, data_bus: DataBus) -> str:
        ...
        edge_info = ""
        if node.edges:
            tool_names = ", ".join(f'"{e.label}(output={{...}})"' for e in node.edges)
            edge_info  = (
                f"\n\nWhen your task is complete, call one of these tools to "
                f"signal the transition: {tool_names}"
            )
        else:
            from agenthicc.workflow.phase_tools import _FINISH_LABEL  # noqa: PLC0415
            edge_info = f"\n\nWhen your task is complete, call {_FINISH_LABEL}(output={{...}})."
        return f"{ctx_block}\n\nTask: {intent}{edge_info}"

    def _find_resume_node(self, data_bus: DataBus) -> str | None:
        completed  = set(data_bus.outputs.keys())
        node_name  = self._def.entry
        seen: set[str] = set()

        while node_name is not None:
            if node_name in seen:
                return node_name   # cycle via rejection — needs re-execution
            seen.add(node_name)
