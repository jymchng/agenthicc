...
class GraphApiOutlookBackend(OutlookBackend):
    ...
    async def _get(self, path: str) -> object:
        ...
        try:
            ...
            if is_network_error(exc):
                raise _OutlookNetworkError(f"{type(exc).__name__}: {exc}") from exc
            raise
    ...
    async def list_emails(
        self, folder: str = "Inbox", n: int = 20, unread_only: bool = False
    ) -> list[dict]:
        params = (
            f"$top={n}&$select=id,subject,from,toRecipients,receivedDateTime,isRead,bodyPreview"
        )
        if unread_only:
    ...
    async def read_email(self, email_id: str) -> dict:
        m = await self._get(
            f"/me/messages/{email_id}?$select=id,subject,from,toRecipients,ccRecipients,receivedDateTime,body"
        )
        return {
            "subject": m.get("subject", ""),
            "from": m.get("from", {}).get("emailAddress", {}).get("address", ""),
    ...
    async def search_emails(self, query: str, folder: str = "Inbox", n: int = 20) -> list[dict]:
        ...
        data = await self._get(
            f'/me/mailFolders/{folder}/messages?$search="{q}"&$top={n}&$select=id,subject,from,receivedDateTime,bodyPreview'
        )
        return [
            {
                "id": m["id"],
...
class OutlookSendEmailTool(_OutlookToolBase):
    ...
    async def execute(self, args, context):
        return await self._backend.send_email(
            args["to"], args["subject"], args["body"], args.get("cc"), args.get("attachments")
        )
...
class OutlookReplyEmailTool(_OutlookToolBase):
    ...
    async def execute(self, args, context):
        return await self._backend.reply_email(
            args["email_id"], args["body"], bool(args.get("reply_all", False))
        )
...
class OutlookSearchEmailsTool(_OutlookToolBase):
    ...
    async def execute(self, args, context):
        emails = await self._backend.search_emails(
            args["query"], args.get("folder", "Inbox"), int(args.get("n", 20))
        )
        return {"emails": emails, "count": len(emails)}
...
class OutlookCreateEventTool(_OutlookToolBase):
    ...
    async def execute(self, args, context):
        return await self._backend.create_event(
            args["subject"], args["start"], args["end"], args.get("attendees"), args.get("body")
        )


class OutlookToolKit:
    def __init__(
        self, backend: str = "auto", token: str | None = None, tenant_id: str | None = None
    ) -> None:
        self._token = token
    ...
    def tools(self) -> list[Tool]:
        ...
        return [
            OutlookListEmailsTool(b),
            OutlookReadEmailTool(b),
            OutlookSendEmailTool(b),
            OutlookReplyEmailTool(b),
            OutlookSearchEmailsTool(b),
            OutlookMoveEmailTool(b),
            OutlookListFoldersTool(b),
            OutlookCalendarEventsTool(b),
            OutlookCreateEventTool(b),
        ]
