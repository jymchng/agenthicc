...
A 22-phase (plus a dynamic per-route PAGE phase) orchestration workflow:
...
def _make_visual_validation_tools(
    ...
    async def visual_rejected(
        ...
        data["target_phase"] = target_phase.strip()
        event.set()
        return {"ok": True}
...
class ReconstructSiteRunner(CodePlanRunner):
    ...
    async def run(self, intent: str) -> ReconstructContext:
        """Drive the 22-phase + dynamic PAGE workflow."""
        from lauren_ai._memory import ShortTermMemory
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    "screenshots where possible and analyse the reference's typography "
    ...
    async def _architecture(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                intent=ctx.intent,
                text=(
                    "Design the target Next.js application architecture before "
                    "implementation."
                    if attempt == 1
    ...
    async def _design_system(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                intent=ctx.intent,
                text=(
                    "Translate the reference visual language into a reusable design "
                    "system."
                    if attempt == 1
    ...
    async def _component_system(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                intent=ctx.intent,
                text=(
                    "Implement the shared component system over customized shadcn/ui "
                    "primitives."
                    if attempt == 1
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    "major route: dimensions, positioning, spacing, typography, colors, "
                ...
                max_turns=35,
                shared_memory=memory,
                tools=_make_visual_validation_tools(event, data),
            )
            if event.is_set():
                action = str(data.get("action", "reject"))
                ctx.validation_status["visual"] = (
                    "approved" if action == "approve" else "rejected"
                )
                ctx.last_transition = "visual_approved" if action == "approve" else "visual_rejected"
                if action == "approve":
    ...
    async def _interaction_validation(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                max_turns=35,
                shared_memory=memory,
                tools=_make_interaction_validation_tools(event, data),
            )
            if event.is_set():
                action = str(data.get("action", "reject"))
    ...
    async def _accessibility(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                max_turns=30,
                shared_memory=memory,
                tools=_make_accessibility_tools(event, data),
            )
            if event.is_set():
                action = str(data.get("action", "reject"))
                ...
                ctx.last_transition = (
                    "a11y_approved" if action == "approve" else "a11y_rejected"
                )
                if action == "approve":
    ...
    async def _performance(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                max_turns=30,
                shared_memory=memory,
                tools=_make_performance_tools(event, data),
            )
            if event.is_set():
                action = str(data.get("action", "reject"))
                ...
                ctx.last_transition = (
                    "perf_approved" if action == "approve" else "perf_rejected"
                )
                if action == "approve":
    ...
    async def _fidelity_pass(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                max_turns=30,
                shared_memory=memory,
                tools=_make_fidelity_pass_tools(event, data),
            )
            if event.is_set():
                action = str(data.get("action", "reject"))
    ...
    async def _final_validation(
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            if event.is_set():
                action = str(data.get("action", "reject"))
                ctx.last_transition = (
                    "final_approved" if action == "approve" else "final_blocked"
                )
                if action == "approve":
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Introduce a SQLite database with realistic mocked/seed data for local development. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify the SQLite database: schema init, deterministic seed, reset/reseed, application consumption, no hard-coded DB records in components, tests/typecheck/lint/build. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Introduce Prisma as the ORM/schema/migration layer over SQLite. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify Prisma schema validation, client generation, migrations (fresh + existing), seed, reset, relations, types, integration, build, typecheck, tests. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Introduce TanStack Query as the client data layer: QueryClient, provider, query functions, mutations, query keys, cache invalidation, loading/error/empty/mutation states. UI must consume data through TanStack Query; Prisma must stay server-only. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify QueryClient/provider/query functions/mutations/keys/invalidation/loading/error/empty states, client/server boundaries, Prisma excluded from client bundles, build, typecheck, tests. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Create and configure .env.local, .env.example, .env.prod, and .env.netlify and update .gitignore. Every variable must have detailed comments (purpose, scope, how to generate/obtain, format, local vs prod). No real secrets. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify all four .env.* files exist, every variable is documented, no secrets, .gitignore behavior, names match code references, build/startup works with the documented local config. "
            ...
            if event.is_set():
                action = str(data.get("action", "rejected"))
                ctx.last_transition = (
                    "env_verified" if action == "verified" else "env_rejected"
                )
                if action == "verified":
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Create a Dockerfile (multi-stage where appropriate), docker-compose.yaml, docker-compose-dev.yaml, .dockerignore, and Docker docs under docs/. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify Docker syntax, compose syntax, image build where available, startup, accessibility, logs, health checks, env, Prisma init, database/volume behavior, no secrets baked in, docs commands work. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Create/refine netlify.toml (build command including prisma generate, publish dir, redirects, headers, functions where needed). Document the SQLite production limitation. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify netlify.toml, build command, publish dir, redirects, headers, functions, plugins, env vars, prisma generation, production build, deployment compatibility, SQLite production limitation. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Create a Caddyfile reverse proxy (documented placeholder domain, HTTPS, headers, compression, WebSocket upgrade where needed) and Caddy docs under docs/. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify Caddy syntax, upstream address, ports, reverse proxy behavior, WebSocket behavior, Docker networking, HTTPS config, docs. "
            ...
            if event.is_set():
                action = str(data.get("action", "rejected"))
                ctx.last_transition = (
                    "caddy_verified" if action == "verified" else "caddy_rejected"
                )
                if action == "verified":
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Enhance package.json with development, quality (lint/typecheck/test/check), database (db:generate/migrate/migrate:deploy/seed/reset/studio), Docker, and deployment commands. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Verify every new command's executable exists, run non-destructive commands, lint, typecheck, tests, production build, Prisma commands, Docker commands, deployment prep, package manager usage. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Create purposeful Bash automation under scripts/ (env, database, development, quality, Docker, deployment, diagnostics) and docs/scripts.md. Every script must be robust and safe. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Run 'bash -n scripts/*.sh', verify executable permissions, run relevant scripts, test from different working dirs, test missing-prereq behavior, verify destructive safeguards, search for secrets, verify docs. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Write comprehensive docs/ covering overview, getting started, architecture, source code, components, data model, Prisma, TanStack Query, env variables, development workflow, testing, Docker, Caddy, Netlify, deployment, bash automation, package commands, troubleshooting, security, operations, reconstruction decisions. Use real commands and paths. "
        ...
        for attempt in range(1, _MAX_ATTEMPTS + 1):
            ...
            await self.run_phase(
                ...
                system_prompt=(
                    ...
                    f"Audit the docs: enumerate source/config/infra files, compare against docs, find undocumented systems, outdated commands, incorrect paths, missing env vars, missing scripts, missing deployment config; verify commands and paths exist; check contradictions; re-read as a new developer. "
            ...
            if event.is_set():
                action = str(data.get("action", "rejected"))
                ctx.last_transition = (
                    "docs_verified" if action == "verified" else "docs_rejected"
                )
                if action == "verified":
