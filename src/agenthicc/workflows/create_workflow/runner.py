...
_RUNNER_GUIDE: str = (
    ...
    "  9a. Expose resolve_checkpoint_topology(context_payload) when the workflow can "
    "filter, skip, generate, or reorder phases. Persist the selected phase names, "
    "profile, and plan/topology version in the typed context, and derive the active "
    "phase index from that resolver. A fixed PhaseSpec list may use the inherited "
    "resolver. Never compare a profile-local index with the full registry list.\n"
...
_GENERATE_PROMPT: str = (
    ...
    "Checkpoint topology is part of the generated workflow contract. For a fixed "
    "PhaseSpec list, the inherited resolver is sufficient. If the runner filters, "
    "skips, generates, or reorders phases, add a pure "
    "resolve_checkpoint_topology(context_payload) method that reconstructs the same "
    "active ordered names and edges from persisted profile/plan/selection fields. "
    "Persist those fields in the checkpoint codec. Use the resolved active name-to-index "
    "mapping for annotations, phase entry, boundaries, failures, and resume; the full "
    "plugin registry is not the active cursor coordinate system.\n\n"
...
class CreateWorkflowRunner(BaseWorkflowRunner):
    ...
    async def run(self, intent: str) -> CreateWorkflowContext:
        ...
        try:
            ...
            if state == CreateWorkflowState.FAILED and handle is not None:
                from agenthicc.runners.workflow_handle import classify_workflow_failure

                failure = ctx.fail_reason or "create_workflow phase failed"
    ...
    async def resume(self, context: object) -> CreateWorkflowContext:
        ...
        try:
            ...
            if final_status == "failed" and handle is not None:
                from agenthicc.runners.workflow_handle import classify_workflow_failure
