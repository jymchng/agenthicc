...
_RUNNER_GUIDE: str = (
    ...
    "text=, system_prompt=, mode=, max_turns=, shared_memory=, tools=) helper; create one "
    "ShortTermMemory in run() and pass it to every call so the phases share context. Never "
...
_DESIGN_PROMPT: str = (
    ...
    "context dataclass fields, one method per state, and the transition tool that ends each "
    "phase. Call describe_runner_pattern() and follow it. Only if every phase is a single "
...
_GENERATE_PROMPT: str = (
    ...
    "tool factories, and build_runner() returning it. Do not leave the runner as a comment, "
