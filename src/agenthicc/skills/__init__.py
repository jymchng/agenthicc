...
class WebSearchSkill(SkillBundle):
    ...
    def system_prompt_addition(self) -> str:
        return "**Web search**: use `search_web(query)` to search and `fetch_page(url)` to read pages."
...
class DockerSkill(SkillBundle):
    ...
    def system_prompt_addition(self) -> str:
        return "**Docker**: manage containers with `run_bash('docker ...')` or dedicated docker tools."
    def tools(self, config: dict) -> list[Tool]:
...
class DatabaseSkill(SkillBundle):
    ...
    def system_prompt_addition(self) -> str:
        return "**Database**: query with `db_query(sql)`, inspect with `db_schema`/`db_list_tables`."
    def tools(self, config: dict) -> list[Tool]:
